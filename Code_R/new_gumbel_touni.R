...
summary(f.t <- fitCopula(gumbelCopula(dim=3, use.indepC="FALSE"), U, method="itau"))
# to.uniform
#Estimate Std. Error
# param   1.494      0.003
# cdf
...
# gofC = gofCopula(gumbelCopula(dim=3, use.indepC="FALSE"), U, method="Sn")

# para_gof = matrix(c(1.39, 34.2, 5e-04), nrow = 1, ncol = 3)
# 
# dimnames(para_gof) = list(c(""), c("parameter", "Statistic", "p-value"))
# 
# write.csv(para_gof, "para_gof_gumbel_touni.csv")

r = mvdc(copula=gumbelCopula(f.t@estimate, dim = 3),margins=c("gamma","lnorm","lnorm"), paramMargins = list(list(shape=shape_X, scale=scale_X),list(meanlog=mu_Y,sdlog=sigma_Y),list(meanlog=mu_Z,sdlog=sigma_Z)))
...
# # shape         rate    
# # 1.820054015   0.251811657 
# # (0.007981024) (0.001269814)
# y.para = fitdistr(y.samp, densfun = "log-normal") 
# # meanlog        sdlog   
# # 2.193233228   0.456392797
# z.para = fitdistr(z.samp, densfun = "log-normal") 
# # meanlog        sdlog   
# # 0.809471349   0.829913921 
...
write.csv(corr1,"gumbel Kendall correlation of the original data.csv")
write.csv(corr2,"gumbel Kendall correlation of the resample data.csv")